...
export type VaccineStatus = 'complete' | 'due' | 'upcoming' | 'overdue'
...
export function computeSchedule(dob: string, loggedVaccines: { vaccineName: string; doseNumber: number | null }[]): ScheduleWithStatus[] {
  ...
  return VACCINE_SCHEDULE.map(entry => {
    ...
    const isComplete = loggedVaccines.some(
      v => v.vaccineName.toLowerCase() === entry.vaccine.toLowerCase() && v.doseNumber === entry.dose
    )
